...
def initialize_population(size, num_antennas, x_bounds=(-20, 20), y_bounds=(0, 0)):
    ...
    Parameters:
        ...
        num_antennas (int): Number of antennas per individual.
        x_bounds (tuple): Min and max values for x-coordinates.
        y_bounds (tuple): Min and max values for y-coordinates.
    
    Returns:
        list of np.ndarray: Population of antenna arrays, each as an (N, 2) array.
    """
    population = []
...
def mutate(antenna_positions, mutation_rate=0.3):
    """Mutate antenna positions with half-wavelength constraints."""
    for i in range(len(antenna_positions)):
        if random.random() < mutation_rate:
            # Ensure new position is still on the grid with half-wavelength spacing
            antenna_positions[i, 0] = round(antenna_positions[i, 0] + random.choice([-1, 1]) * (wavelength / 2), 1)
...
num_antennas = 8
best_positions = evolutionary_algorithm(num_generations=50, population_size=20, num_antennas=num_antennas)
