...
def reward_function(individual, theta_steer=0):
    # Normalize the difference between individual and theta_steer to a range [0, 1]
    max_deviation = np.pi * len(individual)  # Maximum possible deviation
    deviation = np.sum(np.abs(individual - theta_steer))  # Total deviation of the individual
    return 1 - (deviation / max_deviation)  # Higher fitness for lower deviation
...
def genetic_algorithm_with_fallback(num_generations, population_size, num_antennas, regression_tolerance=0.01):
    """Run the Genetic Algorithm (GA) with fitness fallback to prevent significant regression."""
    global positions
    population = initialize_population(population_size, num_antennas)
    mutation_rate = 0.2  # Initial mutation rate

    # Track the best population and fitness from the previous generation
    best_fitness = -np.inf
    best_population = population.copy()

    for generation in range(num_generations):
        # Evaluate fitness of each individual
        fitness = [reward_function(p, theta_steer=0) for p in population]

        # Get the current best fitness and its index
        current_best_fitness = max(fitness)
        best_index = np.argmax(fitness)

        print(f"Generation {generation + 1}, Best Fitness: {current_best_fitness}")

        # Check if fitness has regressed significantly
        if current_best_fitness < best_fitness - regression_tolerance:
            print(f"Significant regression detected! Reverting to previous population.")
            population = best_population.copy()  # Revert to the previous best population
            continue
        else:
            # Update the best fitness and best population
            best_fitness = current_best_fitness
            best_population = population.copy()

        # Update mutation rate dynamically
        mutation_rate = max(0.01, mutation_rate * 0.95)  # Decay mutation rate over generations
        ...
        positions = population[best_index]  # Update global positions to the best solution so far
        update_plot()

        # Allow visualization updates
        plt.pause(0.1)

    # Return the best solution
    best_index = np.argmax([reward_function(p, theta_steer=0) for p in population])
    return population[best_index]
...
def initialize_population(size, num_antennas, x_bounds=(-20, 20), y_bounds=(0, 0)):
    ...
    """
    population = []
    half_wavelength = wavelength / 2  # Minimum spacing

    for _ in range(size):
        x_positions = []
        while len(x_positions) < num_antennas:
            x_pos = np.random.uniform(x_bounds[0], x_bounds[1])
            # Check if the new position is at least half a wavelength away from existing positions
            if all(abs(x_pos - x) >= half_wavelength for x in x_positions):
                x_positions.append(x_pos)

        x_positions = sorted(x_positions)  # Sort for cleaner visualization
        y_positions = [y_bounds[0]] * num_antennas  # Fixed y-coordinates
...
def mutate(antenna_positions, mutation_rate=0.1):
    ...
    for i in range(len(antenna_positions)):
        if random.random() < mutation_rate:
            new_x = antenna_positions[i, 0] + random.choice([-1, 1]) * half_wavelength
            # Ensure the new position doesn't overlap with others
            if all(abs(new_x - antenna_positions[j, 0]) >= half_wavelength for j in range(len(antenna_positions)) if j != i):
                antenna_positions[i, 0] = new_x
...
def crossover(parent1, parent2):
    ...
    # Enforce half-wavelength spacing in the child
    half_wavelength = wavelength / 2
    child[:, 0] = sorted(child[:, 0])  # Sort positions to reduce chances of overlap
    for i in range(1, len(child)):
        if child[i, 0] - child[i - 1, 0] < half_wavelength:
            child[i, 0] = child[i - 1, 0] + half_wavelength
    return child
...
num_antennas = 8
best_positions = genetic_algorithm(num_generations=100, population_size=50, num_antennas=num_antennas)
